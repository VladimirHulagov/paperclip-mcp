    ...
    if resp.status_code >= 400:
        ...
        if resp.status_code == 409:
            ...
            return {
                ...
                "detail": detail,
                "hint": "Issue is locked by another run. Save your work to Outline or disk and ask the CEO/admin to update the issue manually.",
                "conflict_info": body,
            }
...
async def list_issues(
    ...
    if assigneeAgentId:
        if assigneeAgentId == "me":
            assigneeAgentId = _current_agent_id
        if not assigneeAgentId:
            return {"error": "assigneeAgentId=me but agent ID is not available (missing x-paperclip-agent-id header)"}
